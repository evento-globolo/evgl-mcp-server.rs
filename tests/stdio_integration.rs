...
async fn real_process_negotiates_and_lists_namespaced_tools_without_stdout_noise() {
    let mut child = tokio::process::Command::new(env!("CARGO_BIN_EXE_evgl-mcp-server"))
...
"
    ...
    assert!(serialized.contains("evgl_fleet_map"));
    assert!(serialized.contains("evgl_plan"));
}
