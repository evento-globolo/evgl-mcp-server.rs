...
use evgl_mcp_server::{SERVER_NAME, dependency_graph};
use ore_mcp_testkit::{
...
fn initialize_request(id: u64, protocol_version: &str) -> Value {
    json!({
        ...
        "params": {
            ...
            "capabilities": {},
            "clientInfo": {"name": "den-3054-test", "version": "1.0.0"}
        }
    })
}

fn run_session_with_status(requests: &[Value]) -> (ExitStatus, Vec<u8>, Vec<u8>) {
    let mut child = Command::new(env!("CARGO_BIN_EXE_evgl-mcp-server"))
        .stdin(Stdio::piped())
        .stdout(Stdio::piped())
        .stderr(Stdio::piped())
        .spawn()
        .expect("spawn Evento Globolo MCP server");
...
fn official_rmcp_process_preserves_protocol_and_tool_contract() {
    let requests = [
        ...
        json!({
            ...
            "id": 10,
            "method": "tools/call",
            "params": {"name": "evento-globolo/unknown", "arguments": {}}
        }),
    ];
