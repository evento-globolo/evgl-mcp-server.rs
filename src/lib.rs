//! Evento Globolo's read-only MCP server contract.
//!
...
pub const SERVER_TITLE: &str = "Evento Globolo MCP Server";
...
const ORGANIZATION: &str = "evento-globolo";
const REPOSITORY: &str = "evento-globolo/evgl-mcp-server.rs";
const PACKAGE: &str = "evgl-mcp-server";
...
/// Stateless, read-only Evento Globolo MCP handler.
...
pub fn dependency_graph() -> DependencyGraph {
    DependencyGraph::new(ORGANIZATION, REPOSITORY, PACKAGE, DEPENDENCIES)
        .expect("static Evento Globolo dependency graph must remain valid")
}
